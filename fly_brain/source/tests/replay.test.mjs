...
import { parseReplay, frameAt } from './fixtures/replay.ts';
...
test('bundled synthetic example uses actual visible MaleCNS IDs',()=>{
 const bytes=readFileSync(new URL('./fixtures/ids.bin',import.meta.url));
 const groups=readFileSync(new URL('./fixtures/groups.bin',import.meta.url));
 const visible=new Set();for(let i=0;i<groups.length;i++)if(groups[i]<3)visible.add(bytes.readUInt32LE(i*4));
 const example=JSON.parse(readFileSync(new URL('./fixtures/model-output.example.json',import.meta.url),'utf8'));
