...
Functional exploration now uses native HCP-MMP1.0 parcels on matching HCP S1200 fs_LR32k cortical surfaces for 21 cortical topics, plus whole anatomical hippocampi. Whole gyri are no longer used as cortical functional boundaries. See [functional methods, source hashes and noncommercial license](functional/METHODS.md). The functional surface is separate from Z-Anatomy and tractography. Individual functional variability and distributed networks remain explicit limits; independent expert review has not been performed.
